...
void Source::SetPosAng(const TVector3& position, double det, double SD){
    ...
    G4ThreeVector xprime(1- pow(sx,2)/(pow(R,2)+SD*R),
                        -sx*sy/(pow(R,2)+SD*R),sx/R);
    G4ThreeVector yprime(xprime.getY(),
        1- pow(sy,2)/(pow(R,2)+SD*R),sy/R);

    SetAngles(det/2.0, SD);
    fSourceGeant->GetCurrentSource()->GetAngDist()->
    DefineAngRefAxes ("angref1", xprime);
    fSourceGeant->GetCurrentSource()->GetAngDist()->
    DefineAngRefAxes ("angref2", yprime);
...
void Source::SetAngles(const double det, const double SD){
    ...
    double cos1 = (-sx*(det-sx)-sy*(det-sy)+pow(SD,2))/sqrt(pow(sx,2)+pow(sy,2)+pow(SD,2))/sqrt(pow(det-sx,2)+pow(det-sy,2)+pow(SD,2));
    double cos2 = (-sx*(det-sx)-sy*(-det-sy)+pow(SD,2))/sqrt(pow(sx,2)+pow(sy,2)+pow(SD,2))/sqrt(pow(det-sx,2)+pow(-det-sy,2)+pow(SD,2));
    double cos3 = (-sx*(-det-sx)-sy*(det-sy)+pow(SD,2))/sqrt(pow(sx,2)+pow(sy,2)+pow(SD,2))/sqrt(pow(-det-sx,2)+pow(det-sy,2)+pow(SD,2));
    double cos4 = (-sx*(-det-sx)-sy*(-det-sy)+pow(SD,2))/sqrt(pow(sx,2)+pow(sy,2)+pow(SD,2))/sqrt(pow(-det-sx,2)+pow(-det-sy,2)+pow(SD,2));
